  ...
  setup() {
    ...
    const onDragEnd = (event) => {
      ...
      const toSectionIndex =
        event.to.closest(".section")?.dataset?.sectionIndex;

      console.log(
        `Card "${draggedItem.name}" moved from Section ${sections[fromSectionIndex].sectionName} to Section ${sections[toSectionIndex].sectionName}`
      );
    };
    ...
    return { sections, onDragEnd, onDragStart, boardFilters };
  },
});
</script>
